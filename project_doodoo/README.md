...
이전 팀 프로젝트에서 명확한 컨벤션 없이 개발하다가 파일 위치를 두고 팀원 간 혼선이 생겼고, 혼자 만든 코드임에도 몇 주 후에 구조 파악에 시간이 걸리는 경험을 했습니다. 이 프로젝트에서는 시작 전에 **관심사 분리(Separation of Concerns)** 를 기준으로 디렉토리 구조를 먼저 잡고, 규칙을 정한 뒤 개발에 들어갔습니다. 덕분에 기능 추가 시 어느 레이어에 코드를 넣어야 할지 판단이 빨라졌고, 관리자 기능을 뒤늦게 추가할 때도 기존 구조를 깨지 않고 확장할 수 있었습니다.
